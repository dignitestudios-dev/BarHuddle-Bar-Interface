...
export function DeleteGalleryConfirmModal({
    ...
    return (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 md:p-6 animate-in fade-in duration-200 font-['Manrope',sans-serif]">
            <div className="absolute inset-0" onClick={onClose} />

            <div
                className="relative w-full max-w-[460px] rounded-[24px] p-4 sm:p-6 md:p-8 flex flex-col gap-4 sm:gap-6 shadow-[0px_20px_60px_rgba(0,0,0,0.8)] border border-red-500/30 max-h-[92vh] overflow-y-auto custom-scrollbar"
                style={{ background: "rgba(18, 8, 30, 0.96)" }}
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="flex items-start gap-3 sm:gap-4">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-red-500/15 border border-red-500/30 flex items-center justify-center text-red-400 shrink-0 shadow-lg">
                ...
                {/* Thumbnail Preview */}
                {imageUrl && (
                    <div className="relative w-full h-40 sm:h-44 rounded-2xl overflow-hidden bg-black/50 border border-white/10 shadow-inner">
                        <Image
                ...
                <div className="flex flex-col-reverse sm:flex-row sm:items-center justify-end gap-2.5 sm:gap-3 pt-2">
                    ...
                    <button
                        ...
                        disabled={removeGalleryMutation.isPending}
                        className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-xs sm:text-sm font-bold bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 active:scale-95 text-white shadow-[0px_4px_16px_rgba(239,68,68,0.4)] disabled:opacity-50 disabled:pointer-events-none transition-all text-center"
                    >
